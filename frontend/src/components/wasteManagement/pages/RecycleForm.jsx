...
//   // FIXED: Uncommented and added missing variables
//   const districts = [
...
//                   className="font-semibold text-gray-800 mb-2 group-hover:text-green-600">Find Recycle Centers</button>
...
const RecycleSystem = () => {
  ...
  // Determine current page based on URL
  const [currentPage, setCurrentPage] = useState(() =>
    location.pathname.includes('/summary') ? 'summary' : 'form'
  );
  ...
  const getTotalQuantity = () =>
    Object.values(formData.categories).reduce((sum, qty) => sum + qty, 0);

  const getPointsEarned = () => getTotalQuantity(); // 1 point per unit
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        // backend returns { message, form, profile } or similar
        const payload = result.form || result.data || result;
        if (!payload) throw new Error('No form payload returned from server');
            ...
            <div className="bg-white bg-opacity-95 backdrop-blur-sm rounded-2xl shadow-lg p-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
                {Object.entries(categoryDetails).map(([key, details]) => (
                  <div
                    key={key}
                    className={`${details.color} rounded-lg p-3 border-2 border-transparent hover:border-green-300 transition-all duration-200`}
                  >
                    <div className="text-center mb-2">
                      <div className="text-lg mb-1">{details.icon}</div>
                      <h3 className="font-medium text-gray-800 text-sm">{details.label}</h3>
        ...
        <div className="max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <p className="text-gray-600">
                  {submittedData?.location?.city}, {submittedData?.location?.district}
                </p>
              </div>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <p className="text-2xl font-bold" style={{ color: '#5AD670' }}>
                  {submittedData?.pointsEarned}
                </p>
              </div>
            </div>
          ...
          <div className="bg-white bg-opacity-95 backdrop-blur-sm rounded-2xl shadow-lg p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              ...
              {/* Find Recycle Centers (FIXED: single clickable container, no nested button) */}
              <div
                onClick={navigateToRecycleCenters}
                role="button"
                tabIndex={0}
                onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && navigateToRecycleCenters()}
                className="p-6 border-2 border-dashed border-gray-300 rounded-xl hover:border-green-400 hover:bg-green-50 transition-all duration-200 cursor-pointer group"
              >
                <div className="text-center">
                  <div className="text-4xl mb-3">🏪</div>
                  <h3 className="font-semibold text-gray-800 mb-2 group-hover:text-green-600">
