...
const CustomerDashboard = () => {
  ...
  const [loading, setLoading] = useState(true);
  const [totalPoints, setTotalPoints] = useState(0);
  const [profile, setProfile] = useState(null);             // { user, totalPoints, badges, totals }
  const [forms, setForms] = useState([]);                   // history list
  const [stats, setStats] = useState({ byMonth: [], byCategory: {} });
  const [pointsHistory, setPointsHistory] = useState([]);   // optional timeline
  const [error, setError] = useState('');

  // -------- Levels / badges helpers --------
  const levelFromPoints = (pts) => {
    if (pts >= 100) return 'Earth Champion';
    if (pts >= 50)  return 'Eco Star';
    if (pts >= 20)  return 'Eco Learner';
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const [p, f, s, ph] = await Promise.all([
          API.get('http://localhost:5000/api/recycle/profile'),
          API.get('http://localhost:5000/api/recycle'),
          API.get('http://localhost:5000/api/recycle/stats'),
          API.get('http://localhost:5000/api/recycle/points-history'),
        ]);

        if (!mounted) return;

        setProfile(p.data);
        const pts = Number(p.data?.totalPoints ?? 0);
        setTotalPoints(Number.isFinite(pts) ? pts : 0);

        setForms(Array.isArray(f.data) ? f.data : []);
        setStats(s.data || { byMonth: [], byCategory: {} });
  ...
  const monthKeyToLabel = (y, m) => {
    const date = new Date(y, m - 1, 1);
    return date.toLocaleDateString('en-US', { month: 'short', year: 'numeric' });
  };

  const monthlySeries = useMemo(() => {
    // stats.byMonth: [{ _id: {y, m}, totalKg, totalPoints }]
    return (stats?.byMonth || []).map(row => ({
      label: monthKeyToLabel(row._id?.y, row._id?.m),
      kg: Number(row.totalKg || 0),
  ...
  const categoriesPie = useMemo(() => {
    ...
    const entries = [
      ...
      { key: 'regiform', label: 'Regiform' },
    ];

    return entries
      .map(e => ({ name: e.label, value: Number(c[e.key] || 0) }))
      .filter(x => x.value > 0);
  }, [stats]);

  const totals = profile?.totals || { totalForms: 0, totalKg: 0, lastSubmission: null };

  // -------- Your original mock "recent activities" kept (optional) --------
  const recentActivities = [
    {
      id: 'REC-001',
      activity: 'Plastic Bottles Recycled',
  ...
  const statsCards = [
    ...
    },
    {
      ...
      value: loading ? '...' : totals.totalForms || 0,
      icon: Recycle,
      color: 'text-green-600 bg-green-100',
      change: '',
      changeType: 'neutral',
    ...
    {
      name: 'Total Recycled',
      value: loading ? '...' : (totals.totalKg ? totals.totalKg.toFixed(2) : '0.00'),
      icon: Wind,
      color: 'text-green-600 bg-green-100',
      change: '',
      changeType: 'neutral',
  ...
  // -------- Colors for Pie (kept simple / auto) --------
  const PIE_COLORS = ['#22c55e', '#10b981', '#84cc16', '#06b6d4', '#f59e0b', '#8b5cf6', '#ef4444'];
    ...
    >
      ...
      <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="mb-8">
          ...
          <p className="text-green-100 mt-2 text-lg drop-shadow-sm">
            Track your progress, explore events, and see your recycling impact grow. 🌱
          </p>
          {error && (
        ...
        {/* Analytics: charts */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
          {/* Line: Points trend by month */}
          <div className="bg-white/95 backdrop-blur-md rounded-xl shadow-xl border border-white/30 p-6 lg:col-span-2">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="text-xs text-gray-500 flex items-center gap-1">
                <Info className="w-3 h-3" /> 1 point = 1 kg
              </div>
            </div>
            {monthlySeries.length === 0 ? (
              <div className="text-sm text-gray-600">No data yet. Submit your first recycle form to see trends.</div>
            ) : (
              <div className="h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={monthlySeries}>
                    ...
                    <Tooltip />
                    <Legend />
                    <Line type="monotone" dataKey="points" name="Points" stroke="#22c55e" strokeWidth={2} dot={false} />
                    <Line type="monotone" dataKey="kg" name="Recycled (kg)" stroke="#06b6d4" strokeWidth={2} dot={false} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            )}
          ...
          <div className="bg-white/95 backdrop-blur-md rounded-xl shadow-xl border border-white/30 p-6">
            ...
            {categoriesPie.length === 0 ? (
              <div className="text-sm text-gray-600">No category data yet.</div>
            ) : (
              <div className="h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={categoriesPie} dataKey="value" nameKey="name" outerRadius={100} label>
                      {categoriesPie.map((entry, idx) => (
        ...
        {/* Bar: Monthly KG (extra view) */}
        <div className="bg-white/95 backdrop-blur-md rounded-xl shadow-xl border border-white/30 p-6 mb-8">
          <h2 className="text-2xl font-bold text-gray-900 mb-4 flex items-center gap-2">
            <Target className="text-green-600" />
            Monthly Recycled (kg)
          </h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
          ...
          <div className="lg:col-span-2">
            <div className="bg-white/95 backdrop-blur-md rounded-xl shadow-xl border border-white/30 p-6">
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Form ID</th>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">KG</th>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    ...
                    {forms.map((f) => (
                      <tr key={f._id} className="hover:bg-gray-50">
                        <td className="px-4 py-3 text-sm font-medium text-gray-900">#{f.form_id}</td>
                        <td className="px-4 py-3 text-sm text-gray-700">
                          {new Date(f.createdAt).toLocaleString()}
                        </td>
                        <td className="px-4 py-3 text-sm text-gray-700">{f?.location?.district || '-'}</td>
                        <td className="px-4 py-3 text-sm text-gray-700">{f.totalQuantity ?? 0}</td>
                        <td className="px-4 py-3 text-sm text-gray-700">{f.pointsEarned ?? 0}</td>
              ...
              {/* Optional: quick export (CSV) */}
              {/* You can add a button here to export forms to CSV if you want */}
            </div>
          </div>

          {/* Badges */}
        {/* Eco Badges (bigger badges, compact card) */}
{/* Eco Badges (vertical layout) */}
<div className="bg-white/95 backdrop-blur-md rounded-xl shadow-xl border border-white/30 p-5 self-start">
      ...
      <div
        ...
        className={`flex items-center p-3 rounded-lg border transition-all duration-200 ${
          badge.earned
            ...
            : 'border-gray-300 bg-gray-100 opacity-90'
        }`}
      >
        {/* Icon on the left */}
        <div className={`flex items-center justify-center w-10 h-10 rounded-md ${badge.color}`}>
          <badge.icon className="w-5 h-5 text-white" />
        </div>
        ...
        {/* Recent activities (kept) + Events CTA */}
          ...
          <div className="bg-white/95 backdrop-blur-md rounded-xl shadow-xl border border-white/30 p-6">
            ...
            <div className="grid grid-cols-1 gap-4">
              <button
                ...
                className="w-full bg-green-600 text-white py-3 px-4 rounded-lg hover:bg-green-700 transition-colors text-sm font-bold shadow-lg hover:shadow-xl"
              >
                Log Recycling
              </button>
              <button
                onClick={() => navigate('/events')}
                className="w-full bg-white border-2 border-green-200 text-gray-800 py-3 px-4 rounded-lg hover:bg-green-50 transition-colors text-sm font-bold"
              >
                Browse Events
              </button>
              <button
                onClick={() => navigate('/customer-dashboard')} // replace with your redeem page if any
                className="w-full bg-white border-2 border-yellow-200 text-gray-800 py-3 px-4 rounded-lg hover:bg-yellow-50 transition-colors text-sm font-bold"
              >
                Redeem Points
              </button>
            </div>
            <p className="text-xs text-gray-500 mt-4">
              Tip: Submit more recycling forms to unlock higher badges!
            </p>
          </div>
        </div>
      </div>
